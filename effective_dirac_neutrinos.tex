...
{}%
{pppia}% small image here, use pixel for nothing
{}
...
\begin{frame}
  \frametitle{Simplest secluded model with SM-singlet massive chiral fermions from SSB}
\begin{align}
...
Deacrese the number of charges to be assigned to dark matter particles, $\psi_i$ below\only<2>{}
  \begin{align*}
    &\left[ \chi_1,\chi_2,\cdots,\psi_{1},\psi_2,\cdots,\psi_{N'}\right]&\\
    \text{Secluded case} && \invisible<1>{\text{Active case}}\\
...
\begin{frame}
    \frametitle{$\operatorname{U}(1)_{\color{blue}X}$ selection}
...
\begin{frame}
  ...
  A systematic way to find all the \emph{universal} Abelian extensions of the standard model is formulated

  All of them can be reformulated as the solution of
  \begin{align*}
    \sum_{\alpha=1}^N z_\alpha=&0\,,&
    \sum_{\alpha=1}^N z_\alpha^3=&0\,,
\end{align*}
...
Once the physical conditions are stablished, the full set of self-consistent model are found from a simple data-analysis procedure
