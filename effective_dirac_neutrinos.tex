...
\title{Effective Dirac neutrino masses in local Abelian symmetries}
...
{}%
{china}% small image here, use pixel for nothing
{}
...
\collaborators{ Andrés Rivera (UdeA), David Suárez (UdeA), Walter Tangarife (Loyola University Chicago)
...
Any local Abelian extension of the Standard Model can be reduced to a set of integers which must satisfy the gravitational anomaly, $\left[\operatorname{SO}(1,3)\right]^2\operatorname{U}(1)_Y$, and the cubic anomaly, $\left[\operatorname{U}(1)_X\right]^3$ conditions:
\begin{align}
    \label{eq:Dcoond}
...
\item<1-> From a list of $N-2$ integers,  e.g., for $N$ even
  \begin{align}
    \boldsymbol{q}=\left[l_1,l_2,\cdots,l_{n},k_1,k_2,\cdots,k_{n}\right],\qquad n=(N-2)/2\,.
  \end{align}
  in the range $[-m,m]$, build two vector-like solutions of $N$ integers,
  \begin{align}
    \boldsymbol{x}=&\left[{\color{blue}l_1},{\color{red}k_1},\cdots {\color{Green}k_n},
                     -{\color{blue}l_1},-{\color{red}k_1},\cdots -{\color{Green}k_n},
                     \right] &
        \boldsymbol{y}=& \left[0,0,{\color{blue}l_1},\cdots {\color{red}l_n},-{\color{blue}l_1},\cdots -{\color{red}l_n}  \right]
  \end{align}
\item<2-> Obtain a (some times) {\color{blue}non vector-like} solution with $z_{\text{max}}=2m$ 
  \begin{align}
...
\text{  \# of $\boldsymbol{q}$ lists} =(2m+1)^{N-2}=
  \begin{cases}
    9261 \to {\color{blue}3} & N=5 \invisible<1-3>{\to{\color{blue}[1, 5, -7, -8, 9]}}\\
...
\end{align}
{\color{blue} 96\,153 $\to$ 5\,196 multi-component DM ($N=8,12$) $\to$ 28 with two Dirac-fermion DM} \only<2-3>{}
...
Decrease the number of charges to be assigned to dark matter particles, $\psi_i$ below\only<2>{}
  \begin{align*}
    ...
    \text{Secluded case: $\color{red}$} && \invisible<1>{\text{Active case: $\color{blue}$}}\\
...
\end{align*}
$\Phi$ is the complex singlet scalar responsible for the SSB of the anomaly-free gauge symmetry with \only<1>{$D$}\only<2>{$X$}-charge 
\begin{align}
     \label{eq:effcon}
   \invisible<2>{\delta}{\color{Green} \phi}=-\invisible<1>{(}{\color{red}\nu}\invisible<1>{+{\color{blue}m})/\delta}\,, %-\nu-L+h-\delta \phi=0 → \delta \phi= -\nu -L+L-m=-\nu-m
\end{align}
...
%%%%%%%%%%%%%%%
\begin{frame}
  \frametitle{Anomaly cancellation: $\mathcal{X}={\color{magenta}B}$ }
...
\begin{align}
\label{eq:Qzero}
    9{\color{red}Q}=-\sum_{\alpha=N'+1}^{N'+5}z_\alpha=-x'+x''\color{blue}+L+L+L\,,
\end{align}
