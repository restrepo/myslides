...
\text{  \# of $\boldsymbol{q}$ lists} =(2m+1)^{N-2}=
  \begin{cases}
    ...
    \vdots & \vdots\\
    1.6\times 10^{13}\to {\color{blue}65910} & N=12\\
  \end{cases}
\end{align}
}
...
\begin{align}
  \mathcal{L}=i \psi_{i}\overline{\sigma}^{\mu}\mathcal{D}_{\mu}\psi_{i}-\frac{1}{4}F'_{\mu\nu} F^{\prime \mu\nu }+\color{blue} \sum_{i<j}h_{ij} \psi_{i}\psi_j \phi^{(*)} +\text{h.c}
\end{align}
{\color{blue} 96\,153 $\to$ 5\,196 multi-component DM ($N=8,12$) $\to$ 28 with two Dirac-fermion DM}
