...
{}%
{capivaras}% small image here, use pixel for nothing
{}
\arxiv{ arXiv:¿¿¿¿.?????
 ...
 \rowcolors{1}{RoyalBlue!20}{}
  \begin{tabular}{l|c|c|c}\hline
    Field&$SU(2)_L$ & $U(1)_Y$&$U(1)_B$\\\hline
     ...
     $H$      & $\mathbf{2}$ & $1/2$   & $0$   \\
     $\Phi$      & $\mathbf{1}$ & $0$   & $-5$   \\\hline
     $S$      & $\mathbf{1}$ & $0$   & $-5$   \\ \hline
...
\end{equation*}
where $D_L$ is the diffusion constant for $\chi_L$, which is related to the scattering rate $\Gamma_L$ by $D_L=\langle v_{p_z}^2\rangle/3\Gamma_L$. Here, $\langle\,\rangle$ means thermal average. $S_{\cancel{CP}}$ is $CP$-violating source that results from the variation of $\theta$
\begin{eqnarray}
...
\end{eqnarray}
where $v_w$ is the wall's velocity and 
\begin{align*}
...
&  \left(M_\chi^2 \theta' \right)''\,=\, \frac{m_\chi s_0 \lambda \left(-2+\cosh \left(\frac{2z}{L_w} \right) \right)\sin \theta}{L_w^3 \cosh^4 \left(\frac{z}{L_w} \right)}\, .
\end{align*}
