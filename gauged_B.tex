...
\begin{frame}
  \frametitle{Problems}
  \begin{itemize}
\item Standard model (SM)  $m_h\sim 40\text{~GeV}$. \includegraphics{coloremoji/emoji_images/hires/1F61E}
\item Beyond the SM: Source of CP contains fields charged under SM\\ $\to$ too large electric dipole moments \includegraphics{coloremoji/emoji_images/hires/1F629}
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Inert SM-singlet complex scalar field which acquires vev with temperature to have strong electroweak phase transition \includegraphics{coloremoji/emoji_images/hires/1F60A}
\item CP violation (CPV) triggered in dark sectors through SM gauge singlets\\
  $\to$ CPV Yukawa between SM-singlet complex scalar and SM-singlet quiral fermions \includegraphics{coloremoji/emoji_images/hires/1F600}
\end{itemize}
...
$\psi_\alpha $ & $\boldsymbol{1}$& $0$     &  $n_\alpha$   \\
  \end{tabular}
  \caption{$q=l=u=d=e=0$ for $\mathcal{X}=D$ }
    \label{tab:partcont2}
\end{table}
...
%%%%%%%%%%%%%%%
\begin{frame}
  \frametitle{Dark symmetry: $q=l=u=d=e=0$}

  Diophantine equations
  
\begin{align} \label{eq:NN3}
 \sum_{\rho=1}^{N}n_{\rho}=0 \qquad \text{and} \qquad \sum_{\rho=1}^{N}n_{\rho}^3=0\,.
\end{align}
\end{frame}

%%%%%%%%%%%%%%%
\begin{frame}
  \frametitle{Active symmetry}
  If the set of integers has one integer, $m$, repeated three times, the extra gauge Abelian symmetry can be identified as one \emph{active} symmetry, $\operatorname{U}(1)_X$, with $N_{\text{chiral}}=N-3$ right-handed singlet chiral fermions with $X$-charges $n_1,n_2,\ldots n_{N_{\text{chiral}}}$.

They SM $X$-charges be written in terms of $m$ and a free parameter that we choose to be the $X$-charge of the conjugate of the SM lepton doublet, $L$  
\begin{align*}
  u=&\frac{4 L}{3}-m\,,& d=m-\frac{2 L}{3}& \,,& Q=& -\frac{L}{3}\,,& e=& m-2 L\,,&
    h =& L-m\,,
\end{align*}

Ejemplo:
\begin{align*}
  (-1,-1,-1,1,1,1) \to& (-m,-m,-m,m,m,m) \\
  (1,1,1,-4,-4,5) \to & m=1
\end{align*}
\end{frame}
