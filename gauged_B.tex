...
\title{ Radiative seesaw and baryogenesis }
\subtitle{with gauged Lepton number}
...
{}%
{hedwig}% small image here, use pixel for nothing
{}
\arxiv{ arXiv:xxxx.xxxxx
%  arXiv:1812.05523 [PRD], 1906.09685 [PRD], 1907.11938, 1909.09574
}
\collaborators{ W. Novelo, L. Leite, O. Peres (UNICAMP)
...
\end{align}
donde $\theta^j$ son los parámetros del grupo y $T_j$ los generadores.
\end{frame}
...
\begin{align}
  K=-i\,,
\end{align}
que genera el elemento del grupo dilaton, $\operatorname{SO}(1)$, $R(\xi)$
\begin{align}
...
\end{align}
 ...
 Las propiedades de transformación para $\partial_\mu$
\begin{align}
...
De esta manera, un producto escalar invariante entre la cuadriderivada y un cuadrivector de Lorentz $A^\mu(x)$ es
\begin{align}
...
\end{align}
Que se puede representar con la  Ley De Kircchoff:
...
\begin{frame}
    ...
    \begin{column}{0.5\textwidth}
      \begin{itemize}
      \item<1-> $\color{red}B=0$  $\to\color{blue}L=-3$ ($-x'+x''{\color{blue}+3L}=0)$
      \item<2-> Effective neutrino mass $\color{blue}\nu=-6$: $\only<2>{\color{blue}}\phi=-(\nu+L)=9$
      \item<3-> Electroweak-scale vector-like fermions: $\only<3>{\color{blue}}\left(L_L'\right)^\dagger L_R''\Phi^*\to x'=-2,\ x''=7$
      \item<4-> At least two generations of heavy Dirac-fermionic DM: $\only<4>{\color{blue}}\left(\chi_{Li}\right)^\dagger \chi_{Ri}  \Phi^*\to z_3=4,\ z_4=5$        
      \end{itemize}
    \end{column}
    \begin{column}{0.5\textwidth}
      \vspace{1cm}

      %(-6, -6, -6, -3, -3, -3,4, 4, 5, 5, 2, 7, 
      \Large $({\only<1>{\color{blue}}\only<2->{\color{black}}-3, -3, -3},
      {\only<1->{\color{black}}\only<2>{\color{blue}}-6, -6, -6},
      {\only<1->{\color{black}}\only<3>{\color{blue}}2, 7},
      {\only<1->{\color{black}}\only<4>{\color{blue}}4, 4,5,5})$
  ...
  \invisible<1-3>{
    \begin{minipage}[t]{3.0\linewidth}
      \includegraphics[scale=0.4]{T1-3-E-I-A}%
      \hspace{2cm}{{{\color{blue}Unique} solution from $\sim 400,000$!}}
    \end{minipage}
  }
...
\begin{frame}
  ...
  \begin{center}
\only<1>{\includegraphics[scale=0.8]{T1-3-E-I-A}}%
\only<2>{\includegraphics[scale=0.8]{T1-3-E-I-A-1}}%
\only<3>{\includegraphics[scale=0.8]{T1-3-E-I-A-2}}%
\only<4>{\includegraphics[scale=0.8]{T1-3-E-I-A-3}}%
\only<5>{\includegraphics[scale=0.8]{T1-3-E-I-A-4}}%
  \end{center}
\end{frame}
...
\begin{frame}
  \frametitle{Full $\operatorname{U}(1)_{\color{cyan}L}$ model}
  \scriptsize
\begin{table}
  \centering
  \begin{tabular}{l|c|c|c}\hline
    Field&$SU(2)_L$ & $U(1)_Y$&$\operatorname{U}(1)_B$\\\hline
     $u_{Ri}$      & $\mathbf{1}$ & $2/3$   & $u=0$   \\ 
     $d_{Ri}$      & $\mathbf{1}$ & $-1/3$   & $d=0$   \\ 
     $\left(Q_i\right)^\dagger$      & $\mathbf{2}$ & $-1/6$   & $Q=0$   \\ 
     $\left(L_i\right)^\dagger$      & $\mathbf{2}$ & $1/2$   & $L=1$   \\ 
     $e_R$      & $\mathbf{1}$ & $-1$   & $e=-1$   \\ 
     $\left(L'_L\right)^\dagger$      & $\mathbf{2}$ & $1/2$   & $-x'=-2/3$   \\ 
     $e'_R$      & $\mathbf{1}$ & $-1$   & $x'=2/3$   \\ 
     $L''_R$      & $\mathbf{2}$ & $-1/2$   & $x''=-7/3$   \\ 
     $\left(e''_L\right)^\dagger$      & $\mathbf{1}$ & $1$   & $-x''=7/3$   \\ 
     $\nu_{R,i}$   & $\mathbf{1}$ & $0$   & $2$   \\
     $\chi_{R}$      & $\mathbf{1}$ & $0$   & $-5/3$   \\
     $\left(\chi_{L}\right)^\dagger$      & $\mathbf{1}$ & $0$   & $-4/3$   \\ \hline
     $H$      & $\mathbf{2}$ & $1/2$   & $0$   \\
     $\Phi$      & $\mathbf{1}$ & $0$   & $-3$   \\
     $\eta$      & $\mathbf{2}$ & $1/2$   & $2/3$   \\
     $\sigma$      & $\mathbf{1}$ & $0$   & $2/3$   \\
     \hline
  \end{tabular}
  \caption{$i=1,2,3$ normalized Lepton number charges with a global factor $-1/3$.}
...
\begin{align}
M_\chi(z)=  m_{\chi}(z) + |y| \operatorname{e}^{i\theta}|S(z)|\,, 
\end{align}
...
$E_1(x)$ is the error function and $K_2(x)$ is the modified Bessel function of the second kind.
