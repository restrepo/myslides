...
\title{Singlet-Doublet Dark Matter }
\subtitle{and Neutrino Masses }
...
\institute{
  \begin{picture}(100,2)
\put(230,-110){ \includegraphics[scale=0.17]{catedral}}
\end{picture}
...
{}
\arxiv{arXiv:1811.11927 [PRD] and arXiv:1906.09685 [PRD]}
\collaborators{ Carlos Yaguna (UPTC), Julian Calle, Oscar Zapata, Andrés Rivera (UdeA),\\ Walter Tangarife (Loyola University Chicago) }
}

\date{\tiny Sep 11, 2019 - Darkwin - Natal [PDF: \url{http://bit.ly/darkwin2}] } % (optional) \today
...
\section{Previously at tree level}
...
\only<1->{\put(30,160.5){ $\invisible<1>{\langle}\mathbf{SM}\invisible<1>{\rangle}$  }}%
\only<1->{\put(18,115){\begin{overpic}[scale=0.118]{mice}\end{overpic}}}%
\only<2->{\put(38,115){ {\large +}  }}%
\only<2->{\put(0,100){\Large  $m^{\nu}_{\text{Majorana}}=\dfrac{h_{\nu}}{\Lambda} L \cdot H L \cdot H\invisible<1>{\dfrac{\color{red}S}{\Lambda}}$ \invisible<1->{\color{red}(three-level)}   }}%
\only<2->{\put(-32,80){ {\scriptsize Type-I arXiv:1808.03352 } }}%
\only<2->{\put(-10,200){\Large $\mathcal{L}=y\left( N_R \right)^{\dagger}L \cdot \invisible<1>{\langle}H\invisible<1>{\rangle}+  \only<2->{y'{\color{red} \langle S\rangle}}  N_R N_R+\text{h.c} $   }}%
\only<2->{\put(124,218.5){ \Huge \invisible<1>{{\color{red}Lo}{\color{Yellow}cal}} $U(1)_X \color{red} \to  Z_7 $  }}%
\only<2>{\put(204,18){ \begin{overpic}[scale=0.2]{tcdm}\end{overpic}}}%
\end{picture}
\end{frame}
...
\begin{frame}
  ...
  \only<1-4>{  \begin{tabular}{c|c|c|c|l}
    \hline  
    Fields  & $\operatorname{SU}(3)_c$  & $\operatorname{SU}(2)_L$ & $\operatorname{U}(1)_Y $ & $\operatorname{U}(1)_{\only<1-4>{\color{blue}X}\only<5->{\color{red}B-L}}$\\ \hline
    \only<1->{$L $}  &$\mathbf{1}$  & $\boldsymbol{2}$& $-1/2$  &  ${\color{blue}l}\invisible<1-4>{=\color{red}-1}$       \\
$Q $  &$\mathbf{3}$  & $\boldsymbol{2}$& $-1/6$  &  $q\only<2>{=-{\color{blue}l}/{3}                   }\only<3-4>{=-{\color{blue}l}/3   }\only<5>{=\color{red}1/3}$      \\
$d_R $&$\mathbf{3}$  & $\boldsymbol{1}$& $-1/2$  &  $d\only<2>{={\color{Green}e}-4{\color{blue}l}/{3}  }\only<3-4>{=1+2{\color{blue}l}/3 }\only<5>{=\color{red}1/3}$    \\
$u_R $&$\mathbf{3}$  & $\boldsymbol{1}$& $+2/3$  &  $u\only<2>{={-\color{Green}e}+{2 \color{blue}l}/{3}}\only<3-4>{=-1-4{\color{blue}l}/3}\only<5>{=\color{red}1/3}$   \\
$e_R $&$\mathbf{1}$  & $\boldsymbol{1}$& $-1$    &  ${\color{Green}e}\only<2>{                         }\only<3-4>{=1+2{\color{blue}l}   }\only<5>{=\color{red}-1 }$      \\
$H $  &$\mathbf{1}$  & $\boldsymbol{2}$& $1/2$  &  $h\only<2>{=-{\color{Green}e}+{\color{blue}l}      }\only<3-4>{=-1-{\color{blue}l}   }\only<5>{=\color{red}0  }$      \\
                 \invisible<1>{$\sum_{\alpha}\psi_{\alpha} $ } &\invisible<1>{$\mathbf{1}$}  & \invisible<1>{$\boldsymbol{1}$}& \invisible<1>{$0$}     &
\only<1-2>{\invisible<1>{ $\sum_{\alpha} \psi_{\alpha}$}}\only<3->{\cellcolor{black!65}\color{white}$-3$}   \\
  \end{tabular}
}
  \only<5->{  \begin{tabular}{c|c|c|c|l|c|c|c}
    \hline  
    Fields  & $\operatorname{SU}(3)_c$  & $\operatorname{SU}(2)_L$ & $\operatorname{U}(1)_Y $ & $\operatorname{U}(1)_{{\color{red}B-L}}$ &$\operatorname{U}(1)_R$& $\operatorname{U}(1)_D$& $\operatorname{U}(1)_G$\\ \hline
$L $ &$\mathbf{1}$ &$\boldsymbol{2}$& $-1/2$&${\color{blue}l}{=\color{red}-1}$  &$0$  &$-3/2$&  $-1/2$     \\
$Q $  &$\mathbf{3}$  & $\boldsymbol{2}$& $-1/6$  &  $\only<5>{q=\color{red}1/3}$&$0$  &$+1/2$ &  $+1/6$       \\
$d_R $&$\mathbf{3}$  & $\boldsymbol{1}$& $-1/2$  &  $\only<5>{d=\color{red}1/3}$&$+1$ &$0$   &  $+2/3$       \\
$u_R $&$\mathbf{3}$  & $\boldsymbol{1}$& $+2/3$  &  $\only<5>{u=\color{red}1/3}$&$-1$ &$+1$   &  $-1/3$      \\
$e_R $&$\mathbf{1}$  & $\boldsymbol{1}$& $-1$    &  $\only<5>{e=\color{red}-1 }$&$+1$ &$-2$  &  $0$         \\
$H $  &$\mathbf{1}$  & $\boldsymbol{2}$& $1/2$  &   $\only<5>{h=\color{red}0  }$&$-1$ &$+1/2$ &  $-1/2$      \\
{$\sum_{\alpha}\psi_{\alpha} $ } &{$\mathbf{1}$}  & {$\boldsymbol{1}$}& \invisible<1>{$0$}     & \cellcolor{black!65}\color{white}$-3$ &\cellcolor{black!65}\color{white}$-3$&\cellcolor{black!65}\color{white}$-3$&\cellcolor{black!65}\color{white}$-3$  \\
  \end{tabular}
}
  \only<1-2>{\invisible<1>{
  \begin{block}{Linear anomaly cancellation conditions}    
  \begin{align*}
 ...
 &\to& \mathrm{U}(1)_{Y}\left[\mathrm{U}(1)_{X}\right]^{2}\,.
  \end{align*}
  \vspace{-0.1cm}
  \end{block}
}}
\only<3->{
  \tiny
  \begin{block}{$\sum_{\alpha} \psi_{\alpha}^3=-3\,,\qquad  \sum_{\alpha} \psi_{\alpha}=-3$}
  \renewcommand{\arraystretch}{1.4}
      \rowcolors{1}{RoyalBlue!20}{}
  \begin{tabular}{c|l}\hline
    $\left(\only<3>{N_R,N_R}\only<4->{{\color{red}\nu_R,\nu_R}},\psi_{N-2},\cdots\psi_1\right)$     & arXiv \\ \hline
    $\left(-1,-1,-1\right)$&\tiny hep-ph/0611205,  Khalil\\
    $\left(-\dfrac{2}{3},-\dfrac{2}{3},-\dfrac{4}{3},-\dfrac{1}{3}\right)$&\includegraphics[scale=0.015]{colombia}\tiny 1607.04029, Patra, Rodejohann, Yaguna [JHEP] \\
    $\left( -1,-1,-\dfrac{10}{7},-\dfrac{4}{7},-\dfrac{2}{7},\dfrac{9}{7} \right)$ &\includegraphics[scale=0.015]{colombia} \tiny 1808.03352, with Bernal, Yaguna, Zapata [PRD]\\
    \invisible<1-3>{$\color{red}\left(-4,-4,+5\right)$}&\invisible<1-3>{\includegraphics[scale=0.015]{brazil}\tiny \color{red} 0706.0473,  Montero, Pleitez [PLB]}\\
    \invisible<1-3>{$\color{red}\left(-\dfrac{8}{5},-\dfrac{8}{5},-\dfrac{2}{5},-\dfrac{7}{5},\dfrac{10}{5}\right)$}&\invisible<1-3>{\includegraphics[scale=0.015]{colombia}\tiny \color{red} 1812.05523, with Calle, Yaguna, Zapata [PRD]}\\    
    \hline
  \end{tabular}
    \end{block}
}
    ...
    \begin{column}{0.4\textwidth}
      \invisible<1-3>{
        \invisible<5>{
          % \begin{block}
          {General cancellation for}
  \begin{align*}
    \left[\mathrm{U}(1)_{X}\right]^{3} &&  \left[\mathrm{SO}(1,3)\right]^{2} \mathrm{U}(1)_{X}:
  \end{align*}
\begin{align*}
\sum_{\alpha} \psi_{\alpha}^3+3 ({\color{Green}e}-2{\color{blue}l})^3=&0\,, &     \sum_{\alpha} \psi_{\alpha}+3 ({\color{Green}e}-2{\color{blue}l})=&0\,, &
\end{align*}
with $\alpha=1,2,\cdots,N$.
% \end{block}

Set of solutions with
\begin{align*}
  {\color{Green}e}-2{\color{blue}l}=1
\end{align*}}%
\only<4>{\includegraphics[scale=0.38]{nomajo1}}%
\only<5>{\includegraphics[scale=0.38]{nomajo}}
}
...
\section{Beyond Dirac fermion dark matter}
...
\begin{frame}
  \frametitle{From {\only<1>{\color{red}}1210.6350} and
    {\only<2>{\color{red}}1805.02025}: $\Delta N_{\text{eff}}=3 \left( T_{\nu_R}/T_{\nu_L} \right)^4$    }
  \begin{columns}
...
\end{align*}
    ...
    \begin{column}{0.45\textwidth}
      \centering
      \invisible<1>{\includegraphics[scale=0.15]{tree-level-dirac-seesaw}
...
\only<1>{\includegraphics[height=4.1cm]{gT}
...
\only<2>{\includegraphics[height=4.1cm]{D_Neff}
  
  {\tiny with J. Calle and Ó. Zapata, in progress
...
\begin{frame}
  \frametitle{\only<1>{One loop topologies $\operatorname{U}(1)_{B-L}\oplus Z_2 \oplus Z_2 $}\only<2-4>{One loop topologies $\operatorname{U}(1)_{B-L}$ only!
   \invisible<2>{\scriptsize with J. Calle, C. Yaguna, and O. Zapata, arXiv:1812.05523 [PRD]  } }\only<5-6>{
    SD${}^{3}$M+$\sigma_i$ ($i=1,2$)     \invisible<6>{\scriptsize with J. Calle, C. Yaguna, and O. Zapata, arXiv:1812.05523 [PRD]  }  }}
\begin{picture}(320,250)
