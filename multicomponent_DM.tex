...
\title{Gauged Lepton number}
\subtitle{with \emph{dark} matter and \emph{dark} baryogenesis}
...
{}%
{comhep_small}% small image here, use pixel for nothing
{}
...
%         \end{minipage}
%       }}  }}%
\only<3->{\put(260,200){\Large $\color{red}\mathcal{L}=-\frac{1}{4}V_{\mu\nu}V^{\mu\nu}+\color{blue}i \sum_i{\psi_i}^\dagger\cancel{\mathcal{D}} \psi_i$   }}%
\only<3->{\put(280,180){\Large $\color{blue} - \only<3-5>{h}\only<6->{y} \left(\psi_1\psi_2 \only<3-5>{\Phi}\only<6>{S} + \text{h.c} \right) $   }}%
\only<3->{\put(280,218.5){ \Huge {{\color{red}Local}} $\color{red}U(1)_{\mathcal{X}}$  }}%
\only<3->{\put(240,125){
    \begin{minipage}{0.4\linewidth}
      SSB: SM-singlet Dirac fermion\\
      \only<3-5>{dark matter $\color{red}m_{\Psi}=h\langle \Phi \rangle$}\only<6>{$CP$ violation Yukawa $\color{blue}y$}
          ...
          \begin{align*}
            \text{Gauged Symmetry: $\color{red}\mathcal{X}\to \only<3>{D}\only<4->{B}$: }&\invisible<3>{\color{red} q\overline{q}\to Z'\to \text{jets}} \\
            \text{Gauged Symmetry: $\color{blue}\mathcal{X}\to \only<3>{X}\only<4->{L}$: }&
          \end{align*}
    \end{minipage}
  }}%
\only<3->{\put(160,120){\Large $F_{\mu\nu}{\color{red}\ V^{\mu\nu}} $   }}%
\only<3->{\put(120,-10){$\color{red}\overline{\Psi}\Psi=\color{blue}\psi_1\psi_2+\psi_1^\dagger\psi_2^\dagger\to \psi_\alpha\psi_\beta \Phi^{(*)}\,,\qquad \alpha=1,\ldots N\to N>4 $}}
\only<3->{\put(204,0){ \begin{overpic}[scale=0.2]{tcdm1}\end{overpic}}}%

\only<5->{\put(280,0){ \begin{overpic}[scale=0.27]{alert}\end{overpic}}}%
\only<5->{\put(296,40){ {\huge 
        \hspace{0.4cm}\begin{minipage}{3cm}
...
%%%%%%%%%%%%%%%
\begin{frame}
  \frametitle{Anomaly cancellation}
Any \emph{universal} local Abelian extension of the Standard Model can be reduced to a set of integers which must satisfy the gravitational anomaly, $\left[\operatorname{SO}(1,3)\right]^2\operatorname{U}(1)_Y$, and the cubic anomaly, $\left[\operatorname{U}(1)_X\right]^3$ conditions:
\begin{align}
    \label{eq:Dcoond}
...
\item<1-> From a list of $N-2$ integers,  e.g., for $N$ even
  \begin{align}
    \boldsymbol{q}=\only<1-3>{\left[l_1,l_2,\cdots,l_{n},k_1,k_2,\cdots,k_{n}\right],\qquad n=(N-2)/2\,.}%
                   \only<4->{\left[2,3,-1,-3\right],\qquad n=6\,.}
  \end{align}
...
\text{  \# of $\boldsymbol{q}$ lists} =(2m+1)^{N-2}=
  \begin{cases}
    9261 \to {\color{blue}3} & N=5 \invisible<1-3>{\to{\color{blue}[1, -2, -3, 5, 5, -6]}}\\
    194841 \to {\color{blue}38}& N=6\\
    \vdots & \vdots\\
...
\begin{frame}
  \frametitle{Minimal secluded model with SM-singlet massive chiral fermions from SSB: $\operatorname{U}(1)_{\color{cyan}D}$}
\begin{align}
...
\begin{align}
  ...
  +{\color{red}  \underbrace{\sum_{a,b=1}^{2}h_{(-2a,-7b)}\psi_4\psi_{-7}\phi}_{\displaystyle\text{\fcolorbox{blue}{yellow}{multi-flavor DM}}}}
...
\begin{frame}
  \frametitle{Secluded Scotogenic model with SM-singlet massive chiral fermions from SSB: $\operatorname{U}(1)_{\color{magenta}D}$}

$\displaystye{LLHH \to LLHH{{\color{red}\Phi \Phi^*}}$
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
  \frametitle{Anomaly cancellation: $\mathcal{X}=\color{cyan}X$}
The anomaly-cancellation conditions on $\left[\operatorname{SU}(3)_c\right]^{2}  \operatorname{U}(1)_{X}$, $\left[\operatorname{SU}(2)_{L}\right]^{2} \operatorname{U}(1)_{X}$, $\left[\operatorname{U}(1)_{Y}\right]^{2} \operatorname{U}(1)_{X}$,  allow us to express three of the $X$-charges in terms of the others\only<2-3>{}
...
\end{align}
 ...
 \vspace{-1cm}
   \invisible<1>{   
 \begin{itemize}
 \item If: $x'=x''$ or $x'=x''=0$ 
 \item We need $\color{blue}h=-e-L=L-m$:
 \end{itemize}
\begin{align*}
  {\color{red}u}=&\frac{4 {\color{blue}L}}{3}-m\,,& {\color{red}d}=m-\frac{2 {\color{blue}L}}{3}& \,,& {\color{red}Q}=& -\frac{{\color{blue}L}}{3}\invisible<1-2>{\ne 0}\,,& {\color{blue}e}=& m-2 {\color{blue}L}\,,&
    h =& {\color{blue}L}-m\,,
\end{align*}
...
\begin{frame}
  \frametitle{Multi-generational dark matter}
\begin{figure}
    \centering
...
\begin{frame}
    \frametitle{$\operatorname{U}(1)_{\color{cyan}X}$ selection}
  ...
  \end{columns}
  \invisible<1-4>{\only<5>{{\color{red}$1\,122$} solutions from $\sim 400,000$}

%\includegraphics[scale=0.45]{dmz}    \includegraphics[height=4.5cm]{u1blc5}
...
\invisible<1>{$S$      & $\mathbf{1}$ & $0$   & $s$}\\ 
  \end{tabular}
  \caption{ \invisible<1>{minimal set of
...
%%%%%%%%%%%%%%%
\begin{frame}
  \frametitle{Anomaly cancellation: $\mathcal{X}={\color{cyan}L}$ }
...
\begin{frame}
    \frametitle{$\operatorname{U}(1)_{\color{cyan}L}$ selection}

      \begin{columns}
    \begin{column}{0.6\textwidth}
      \begin{itemize}
      \item<1-> $\color{red}B=0$ with $\color{blue}L=6$
      \item<2-> Electroweak-scale vector-like fermions with $\Phi=18$: $\only<2>{\color{red}}\left(L_L'\right)^\dagger L_R''\Phi\to x'=8,\ x''=-10$
      \item<3-> $\color{blue}L+L+L\color{red}-x'+x''=0$ 
      \item<4-> Dirac-fermionic DM: $\only<4>{\color{Green}}\left(\chi_L\right)^\dagger \chi_R'\Phi^*\to z_3=5,\ z_4=13$
      \item<5-> (Two generations) Majorana-fermionic DM: $\only<4>{\color{Green}}\left(\chi''_i\right)^\dagger \chi''_j\Phi\to z_5=-9,\ z_6=-9$        
      \end{itemize}
    \end{column}
    \begin{column}{0.4\textwidth}
      \vspace{1cm}
      
      \Large $({\only<1->{\color{blue}}6, 6, 6},
      {\only<1->{\color{black}}\only<2-3>{\color{red}}-8, -10},
      {\only<1->{\color{black}}\only<4>{\color{Green}}5, 13},
      {\only<1->{\color{black}}\only<5>{\color{Green}}-9, -9}
      )$
  ...
  \invisible<1-5>{\only<6>{Only {\color{red}4} solutions from $96\,153$}    }

  \invisible<1-5>{
    \begin{minipage}[t]{3.0\linewidth}
      \includegraphics[scale=0.7]{foursltns}
    \end{minipage}
  }
...
\begin{frame}
  ...
  A methodology to find all the \emph{universal} Abelian extensions of the standard model is designed
  ...
  \begin{align*}
    ...
    \sum_{\alpha=1}^N z_\alpha^3=&0\,,
\end{align*}
which we fully scan until $N=12$ and $\left| z_{\text{max}} \right|=20$

Once the physical conditions are stablished, the full set of self-consistent models are found from a simple data-analysis procedure
