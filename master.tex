...
\url{http://gfif.udea.edu.co}      
    ...
    \begin{column}{0.3\textwidth}
      \hfill MOCA\\
      \hfill \includegraphics[scale=0.35]{seneca}\\
      \hfill {\tiny{\textcopyright Manuel Vieda}}
    \end{column}
  \end{columns}
\quad\\
\quad\\
...
\date{\tiny June 27, 2017 } % (optional) \today
...
\includegraphics[height=\fh,width=7cm]{ssi_Mdm}
  \caption{$M_{11}=50\ \text{TeV}$ $2.7<M_{23}/\text{TeV}<3.1$ \qquad\qquad\qquad\qquad  (Right:  $\tan\gamma>5$)}
\end{figure}
\end{frame}
...
\begin{frame}
  \frametitle{Ernest Ma, D.R, Óscar Zapata:  \invisible<1-2>{$\Longrightarrow R=(-1)^{2s+3B+L}$} \hfill arXiv:706.08240}
  \begin{center}
...
\begin{frame}
  \frametitle{ISR+$E_T^{\text{miss}}$ \hfill ATLAS-CONF-2017-022}
  
\only<1>{\includegraphics[scale=0.22]{atlas1}}%
\only<2>{\includegraphics[scale=0.22]{atlas2}}%
