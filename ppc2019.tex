...
{mt}%
{}
\arxiv{1812.05523 [PRD] and 1905.NNNNN}
\collaborators{ Carlos Yaguna (UPTC), Julian Calle, Oscar Zapata, Andrés River (UdeA), Walter Tangarife (Loyola University Chicago)  }
}

\date{\tiny May 17, 2019 } % (optional) \today
...
\only<1->{\put(0,100){ $\frac{1}{\Lambda} L \cdot H L \cdot H$   }}%
\only<1->{\put(-10,80){ $\frac{1}{\Lambda} \left( \nu_R \right)^{\dagger}L \cdot H S$  }}%
...
\begin{frame}
  \frametitle{\only<1>{One loop topologies $\operatorname{U}(1)_{B-L}\oplus Z_2 \oplus Z_2 $}\only<2-4>{One loop topologies $\operatorname{U}(1)_{B-L}$ only!}\only<5>{
...
% 
\only<3>{\put(160,220){\tiny $\longmapsto$ generalization to two and three loops: Shaikh Saad arXiv:1902.07259  }}%
