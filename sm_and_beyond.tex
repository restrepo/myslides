...
\begin{columns}
  \begin{column}{0.8\textwidth}
    \begin{block}{\only<1-3>{Not mass, }\only<1>{not charge}\only<2-3>{\color{cyan}hypercharge, }\only<3>{\color{magenta} not Dirac}%
        \only<4-5>{Higgs mechanism with tachyonic mass ($\color{red}\mu^2<0$) and condensate}%
...
i\overline{\psi}\gamma^{\mu}\partial\psi -m_{e}\overline{\psi}\psi
                  -{\color{magenta} i \left( \nu_L \right)^{\dagger}\overline{\sigma}^{\mu}\partial_{\mu}\nu_L }
  + \left( e_R \right)^{\dagger} \Phi^{\dagger} L
                  -\left( D^{\mu}\Phi \right)^{\dagger} D_{\mu}\Phi-{\color{red}\mu^2} \Phi^{\dagger}\Phi -\lambda \left( \Phi^{\dagger}\Phi  \right)^{2}
{\color{blue}- \mathcal{L}_{\text{gauge}}+Z\nu \nu+  m_{e}\overline{\psi}\psi H}
\end{align*}
}
