...
\date{\tiny Novemeber 22, 2018 } % (optional) \today
...
\rowcolors{1}{RoyalBlue!20}{}
    \begin{tabular}{ll}
     Left Field &  $\operatorname{U}(1)_{B-L}$ \\\hline
      ...
      $\left( \nu_{R_2} \right)^{\dagger}$ & $+1$ \\  
      $\xi$ & $\only<1>{-n}\only<2>{-r}$ \\
      $\eta$ & $\only<1>{n}\only<2>{r}$ \\\hline
      $\phi$ & $2$ \\
    \end{tabular}
...
\begin{frame}
  \frametitle{Double Dirac fermion dark matter model}
...
\begin{tabular}{ll}
    ...
    \begin{align*}
      ...
      \chi=& | \mathcal{Q} \mathcal{Q}  \rangle
    \end{align*}
    \end{block}
  \end{column}
  \begin{column}{0.62\textwidth}
...
\begin{frame}
  \frametitle{Total lepton number conservation}
  In the near future \alert{total} lepton number conservation could be established.

  \begin{itemize}
  \item   If $L$ is a conserved quantum number, it must be related to a gauge symmetry
  \item   $Z'$ must be massive and consequently it must be an spontaneously broken gauge symmetry
  \item   An accidental global symmetry for $L$ is left after symmetry breaking.
  \end{itemize}
  
  \begin{block}{  What is the minimal model with Lepton number as a gauge symmetry?}
  ...
  \begin{align}
    ...
    \text{SM + Total Lepton number conservation}
  \end{align}

where $B$ is the \emph{total baryon number}.
\end{frame}

\begin{frame} 
  \frametitle{Small Dirac neutrino masses}
  \small
  To explain the \alert{smallness} of Dirac neutrino masses, usually
  \begin{itemize}
  \item<1-> $Z_2$  to avoid tree-level mass (TL) term ( $Y(H)=+1/2$ )
    \begin{align*}
      \mathcal{L}_{\text{T.L}}=&h_{D}\epsilon_{ab} \left( \nu_R \right)^{\dagger} L^a H^b +\text{h.c}\\
      =&h_D\left( \nu_R \right)^{\dagger} L H +\text{h.c}\\
    \end{align*}
  \item<2-> $Z_2'$ to forbid Majorana term: $\nu_R \nu_R$
   \item<3-> (Local)  $\operatorname{U}(1)_{B-L}   \color{red}  \underbrace{\longrightarrow}_{\left\langle S \right\rangle}
 Z_2''$, with $L(\nu_R)=-1$.
   \item<4-> Realization of the 5-dimension operator which conserves lepton number in    $\operatorname{SU}(3)_c \times\operatorname{SU}(2)_L\times \operatorname{U}(1)_Y\times \operatorname{U}(1)_{B-L}$:
     \begin{align*}
       \mathcal{L}_{5-D}= \frac{h_\nu }{\Lambda}\left( \nu_R \right)^{\dagger} L H
       {\color{red}S}+\text{h.c}\\
     \end{align*}
   \item<5> Prediction of extra relativistic degrees of freedom $N_{\text{eff}}$
  \end{itemize}
...
\section{One-loop realization of $\mathcal{L}_{5-D}$}
\begin{frame}
