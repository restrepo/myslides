...
\begin{frame}
  ...
  \item   An accidental symmetry for total $L$ is left after symmetry breaking.
  \end{itemize}
...
\begin{frame}
  \frametitle{
    ...
    % \only<4>{Exotic $\left( \nu_R \right)^{\dagger}$ with $\nu\ne-1$,
    %  and vector-like Dirac fermion with $r\ne 1$}%
     \only<1>{The model: \textit{colored scotogenic}}%
  }
\def\pscale{0.6}
\def\px{0}
\def\py{140}
\begin{picture}(320,250)
\only<1>{\put(\px,\py){\includegraphics[scale=\pscale]{dm4}}}%
\FPeval\sx{\px+270}%
\FPeval\sy{\py+60}%
\only<1->{\put(\sx,\sy){
    \begin{minipage}{0.4\linewidth}
    ...
    \begin{align*}
      \mathcal{L}\supset & \kappa  \sigma \eta^{\dagger} H\,,
    \end{align*}
    \invisible<1>{where $\kappa=\lambda \langle S \rangle\,.$}
...
\only<1->{\put(0,80){
    \begin{columns}
      \begin{column}{0.55\textwidth}
        ...
        \end{align*}
        \begin{align*}
          \only<1>{\color{red}\boldsymbol{N}_c=\boldsymbol{8}\,.}%
        \end{align*}
      \end{column}
      \begin{column}{0.45\textwidth}
...
\hline
 ...
 $H$                           &          0  & $ \left(\mathbf{1},\mathbf{2}\right)_{1/2}$ \\     
 $\left( \nu_{Ri} \right)^{\dagger}$ & $\color{Green}\nu$ & $ \left(\mathbf{1},\mathbf{1}\right)_0$ \\ 
    \only<1->{$\color{red}\mathcal{Q}_L$}    & $\color{red}-r$ & $ \left(\only<1>{{\color{red}\mathbf{N}_c}},\mathbf{1}\right)_0$\\
 \only<1->{$\color{red}\left( \mathcal{Q}_R \right)^{\dagger}$}   & $\color{red}r$ & $ \left(\only<1>{{\color{red}\mathbf{N}_c}},\mathbf{1}\right)_0$ \\
    $\sigma_a$                           &  ${\color{Green}\nu}-\color{red}r$   & $ \left(\only<1>{{\color{red}\mathbf{N}_c}},\mathbf{1}\right)_0$ \\
    $\eta_a$                        &  $1-\color{red}r$   & $ \left(\only<1>{{\color{red}\mathbf{N}_c}},\mathbf{2}\right)_{1/2}$ \\
    \invisible<1>{$S$}                        &  \invisible<1>{ ${\color{Green}\nu}-1$ }  & $  \invisible<1>{ \left(\mathbf{N}_c,\mathbf{2}\right)_{1/2}$ }\\  
  \end{tabular}
...
\begin{align}
 ...
 &\sim 0.03\,\mbox{eV}\left(\frac{M_{\mathcal{Q}}}{12.5\, \mbox{TeV}}\right)\left(\frac{\kappa^{aa}}{1\, \mbox{GeV}}\right)\left(\frac{50\,\mbox{TeV}}{\mu_\eta^{aa}}\right)^2\left(\frac{h_{i}^a y_{j}^a}{10^{-6}}\right).\nonumber
%
\end{align}
%
...
\begin{frame}
  \frametitle{Some years from now$\cdots$}
    \includegraphics[scale=0.42]{headline1} \hspace{1cm}  \includegraphics[scale=0.82]{DirectBounds}
\end{frame}
...
\begin{frame}
  ...
  \begin{columns}
    ...
    \begin{column}{0.5\linewidth}
      ...
      \end{block}
      \begin{align*}
        \sqrt{s}=85\ \text{TeV needed to discover } M_{\mathcal{Q}}=12.5\ \text{TeV}\,.
      \end{align*}
    \end{column}
  \end{columns}
\end{frame}
