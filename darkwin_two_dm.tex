...
\only<2->{\put(0,100){\Large  $m^{\nu}_{\text{Majorana}}=\frac{1}{\Lambda} L \cdot H L \cdot H$ \invisible<1,3->{\color{red}(1-loop)}   }}%
\only<3->{\put(-10,80){\Large $m^{\nu}_{\text{Dirac}}=\frac{1}{\Lambda} \left( \nu_R \right)^{\dagger}L \cdot H S$  }}%
\only<2>{\put(-32,80){ {\scriptsize arXiv:1308.3655  [JHEP] with C. Yaguna and Ó. Zapata} }}%
\only<2>{\put(204,158){ \begin{overpic}[scale=0.27]{alert}\end{overpic}}}%
\only<2>{\put(224,195){ {\huge \textbf{35 models}}  }}%
\only<4->{\put(124,218.5){ \Huge Local $U(1)_X \color{red} \to  Z_N $  }}%  
\end{picture}
\end{frame}
...
\begin{frame}
  \frametitle{Unification: $\operatorname{SO}(10)$}
  \begin{columns}
    \begin{column}{0.7\textwidth}
      \begin{small}
  \begin{align*}
   \includegraphics[scale=0.07]{mice}= \mathbf{16}_{F_{i}}=
    \begin{pmatrix}
      \color{green}\left(u_{R}\right)^{\dagger} \\
      \color{blue}\left( u_{R}\right)^{\dagger} \\    
      \color{red}\left(  u_{R}\right)^{\dagger}\\    
      \color{green}u_{L}\\
      \color{blue}u_{L}\\
      \color{red}u_{L}\\
      \color{green}\left(d_{R}\right)^{\dagger} \\    
      \color{blue}\left( d_{R}\right)^{\dagger} \\    
      \color{red} \left( d_{R}\right)^{\dagger} \\
      \color{green}d_{L}\\
      \color{blue}d_{L}\\
      \color{red}d_{L}\\
      \nu_L\\
      e_L\\
      \left( e_{R} \right)^{\dagger}\\
      \only<1>{N}\only<2>{\left( \nu_R \right)^{\dagger}}\\
...
\begin{frame}
  \frametitle{$\operatorname{SO}(10)$ breakings}
  \begin{align*}
    \operatorname{SO}(10) \to
  \begin{cases}
    \operatorname{SU}(5)  \times \operatorname{U}(1)_X\, \text{\hspace{5cm}with }
    \begin{cases}
      \text{$Z_X$ at GUT: } \operatorname{U}(1)_X\to Z_N\\
      \text{$Z_X$ at EW}
    \end{cases}
\\
    \operatorname{SU}(3)_c \times \operatorname{SU}(2)_R\times \operatorname{SU}(2)_L \times \operatorname{U}(1)_Y \times \operatorname{U}(1)_{B-L}
  \end{cases}
  \end{align*}
\end{frame}

\section{Majorana neutrinos case}
...
\only<2>{\put(0,70){
  \begin{minipage}[t]{1.0\linewidth}
    ...
    \end{align*}
    arXiv:0912.1545 [PRD] (Frigerio-Hambye) %check for second doublet
  \end{minipage}
}
}
\only<3>{\put(0,70){
...
\centering
    ...
    %uniftwo451E16
    arXiv:1509.06313 [PRD] with C. Arbelaez, R. Longas, and O. Zapata.
  \end{minipage}
}
}
%%%circle
...
\invisible<1-2>{+{\mathcal{L}(\alert{\mathbf{10}_\Phi}\invisible<3>{\ \text{or}\   \mathbf{16}_\Phi})}.} 
\end{align*}
...
\end{align*}
    ...
    \begin{column}{0.53\textwidth}
      ...
      \only<3>{\includegraphics[scale=0.53]{uniftwo451E16}} 
      }%
    \invisible<1>{\only<1-2>{\tiny  \quad \\ S. Horiuchi, O. Macias, DR, A. Rivera, O. Zapata, 1602.04788 (JCAP)}%
                 \only<3>{\tiny \quad \\   (See previous  arXiv:1509.06313 [PRD]):  Split-SUSY:  like $\alert{M_{\Phi}}=2\ \text{TeV}$}%
                 }
    \end{column}
  \end{columns}
...
{ $\mathbf{10}_F \to \psi_L,\left( \psi_R \right)^{\dagger}$\\
                  ...
                  $\mathbf{45}'_F \to N$\\
  \invisible<1->{\tiny arXiv:1511.06495 [PR], N. Arkani-Hamed, T. Han, M. Mangano, LT Wang: signals at a 100~TeV $p$-$p$ collider
                  }
}
\end{frame}
...
\frametitle{Not-susy $\operatorname{SO}(10)\to \operatorname{SU}(3)_c \times \operatorname{SU}(2)_R\times \operatorname{SU}(2)_L \times \operatorname{U}(1)_Y \times \operatorname{U}(1)_{B-L} $}

\textsl{Fermionic triplet dark matter in an SO(10)-inspired left right model}, with 
C. Arbeláez and M. Hirsch, arXiv:1703.08148 [PRD]
...
\begin{frame}
  ...
  \begin{figure}
    ...
    \only<1>{\includegraphics[width=0.55\textwidth]{IIMDMvsMZRExa5}}
...
\begin{frame}
  ...
  \centering
  \includegraphics[width=0.53\textwidth]{ParamSpaceI}
...
\section{Dirac neutrinos case: $\operatorname{SO}(10)\to \operatorname{SU}(5)\times U(1)_X $}
...
\begin{frame}
  \frametitle{\only<1>{One loop topologies $\operatorname{U}(1)_{B-L}\oplus Z_2 \oplus Z_2 $}\only<2->{One loop topologies $\operatorname{U}(1)_{B-L}$ only!
  ...
  \only<2>{\put(280,130){\tiny with J. Calle, C. Yaguna, and O. Zapata, arXiv:1812.05523 [PRD]  }}%
    \only<2-4>{\put(-20,130){\tiny
        \begin{minipage}{0.3\linewidth}
          \begin{align*}
            \psi_{L,R}\to&\text{Singlet fermions \only<3>{(vector-like)}\only<4>{(quiral)}  }\\
...
% 
\only<3>{\put(160,220){\tiny $\longmapsto$ generalization to two and three loops: Shaikh Saad arXiv:1902.07259 [NPB]  }}%
...
\begin{frame}
  \frametitle{    Scotogenic Dirac from $\operatorname{SO}(10)$ E. Ma arXiv:1901.09091 [PLB]    }
\begin{picture}(320,250)
\only<1>{\put(-20,130){\tiny
        \begin{minipage}{0.3\linewidth}
          \begin{align*}
            \psi_{L,R}\to&\text{Singlet fermions \only<1>{(quiral)}  }\\
            \invisible<1>{\Psi_{L,R}\to&\text{Vector-like doublet fermions}   }\\
            \sigma\to&\text{Singlet scalar}   \\
            \only<1>{\eta\to&\text{Doublet scalar}   }
          \end{align*}
        \end{minipage}
}}%
\only<1>{\put(214,135){\tiny
    \begin{minipage}{0.3\linewidth}
      \rowcolors{1}{RoyalBlue!20}{}
  \begin{tabular}{c|cccccccc}\hline
 Fields: $f_i$  &  $\left( \nu_{R3} \right)^{\dagger}$
  &$\left( \nu_{R2} \right)^{\dagger}$&$\left( \nu_{R1} \right)^{\dagger}$&$\psi_L$&$\left( \psi_R \right)^{\dagger}$&$\sigma$&$\eta$&$S$\\ \hline
  $\operatorname{U}(1)_{B-L}$&$\color{Yellow}+4$&$\color{Yellow}+4$&$-5$& $\color{red}-r$ & $\color{red}r$ &$4-\color{red}r$ & $1-\color{red}r$ & $+3$\\
     $\operatorname{SO}(10)$ &$\displaystyle{16}$&$\displaystyle{16}$&$\displaystyle{16}$&$\displaystyle{126}$&$\color{red}\displaystyle{126^{*}}$&$\displaystyle{16}$&$144$&$\color{blue}672$\\
  \end{tabular}
         \end{minipage}
       }}%
\only<1>{\put(300,55){\tiny
    \begin{minipage}{0.3\linewidth}
       \begin{center}
         \textbf{Anomaly cancellation conditions}
         \begin{align*}
...
%  effective number of relativistic degrees of reedom Neff for neutrinos
% \only<4>{\put(40,20){\begin{overpic}[scale=0.6]{radiativedirac4}\end{overpic}}}%
 \only<1->{\put(40,20){\begin{overpic}[scale=0.6]{radiativedirac5-1}\end{overpic}}}%
\end{picture}
\end{frame}
