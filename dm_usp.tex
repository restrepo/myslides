...
\title{Minimal Scotogenic (DM) models}
...
\institute{
  \begin{picture}(100,2)
\put(230,-110){ \includegraphics[scale=0.17]{sao2}}
\end{picture}
...
\date{\tiny Oct 11, 2019 - USP  [PDF: http://bit.ly/darkusp] } % (optional) \today
...
\begin{frame}
  ...
  \begin{block}{Agrawal, \emph{et al}, arXiv:1610.04611 [JCAP]}
      \begin{quote}
...
\begin{align}
  \mathcal{L}=-\frac{1}{4}V_{\mu\nu}V^{\mu\nu}+i \overline{X}i\cancel{\mathcal{D}} X-m \overline{X}X\,.
\end{align}
Relic abundance by $X\overline{X}\to \gamma_{\mathcal{D}}\gamma_{\Delta}$
  \end{block}
\end{frame}
...
with solution 
\begin{align}
  u=&\only<2-4>{-e+\frac{2{\color{blue}l}}{3}}\only<5->{-\only<5>{{\color{Green}r}}\only<6->{{1}}-\frac{4{\color{blue}l}}{3}}\,,& d=& \only<2-4>{e-\frac{4{\color{blue}l}}{3}}\only<5->{\only<5>{{\color{Green}r}}\only<6->{{1}}+\frac{2{\color{blue}l}}{3}}\,,& q=& -\frac{{\color{blue}l}}{3}\,,
  &\invisible<2-4>{e=}&\invisible<2-4>{\only<5>{{\color{Green}r}}\only<6->{{1}}+2{\color{blue}l}\,,}
\end{align}}
\invisible<1-2>{
which satisfy
\begin{align}
...
\invisible<1-3>{
  For $N$ extra quiral fields $\psi_\alpha$ ($\alpha=1,\ldots,N$) with $X$-charges $n_\alpha$:
  \invisible<4>{${\color{Green}r}\equiv e-2{\color{blue}l}\invisible<1-5>{=1}$}
\begin{align*}
  \left[\mathrm{SO}(1,3)\right]^{2} \mathrm{U}(1)_{X}:&&  \sum_{\alpha} n_{\alpha}+3 \only<4>{(e-2{\color{blue}l})}\only<5>{{\color{Green}r}}\only<6->{\ }=&0\,,\\
  \left[\mathrm{U}(1)_{X}\right]^{3}\,,&&\sum_{\alpha} n_{\alpha}^3+3 \only<4>{(e-2{\color{blue}l})^3}\only<5>{{\color{Green}r^3}}\only<6->{\ }=&0\,    
\end{align*}
\invisible<2-4>{
  \only<5>{Then the general anomaly free two-parameter
solution can be written as
}\only<6->{Since $f\to f'\to f/{\color{Green}r}$, without lost of generality:  ${\color{Green}r}\to 1$ }
\begin{align*}
  X(\invisible<6->{{\color{Green}r},}{\color{blue}l} )=\only<5>{{\color{Green}r}}\only<6->{\ } R + {\color{blue}l} Y\,.
\end{align*}
}
}
\invisible<1-5>{
We impose $\nu_{R1}=\psi_N$, $\nu_{R2}=\psi_{N-1}$, to have at most one massless neutrino.
}
\invisible<1-5>{}
\end{frame}
...
%$S$ & $\boldsymbol{1}$ & $0$  & $ 2 \psi_N$& $\color{red}2 \psi_N$&  $2 \psi_N$ & $2 \psi_N$& $2 \psi_N$ &$0$ \\
 ...
 $\sum_\alpha n_{\alpha}^{3}$ & $\boldsymbol{1}$ & $0$ & $-3$& $\color{red}-3$&  $-3$ & $-3$& $-3$&$0$\\ \hline
  ...
  {\color{Yellow}*}  ${\color{Green}r}={\color{blue}l}=0$ ($n_{\alpha}\ne 0$)
...
\begin{frame}
\frametitle{  Known solutions with $\sum n_{\alpha}=-3$ and $\sum n_{\alpha}^3=-3$}
\begin{columns}
...
\begin{column}{0.4\textwidth}
  \only<2>{\includegraphics[scale=0.34]{nomajo1}}%
  \only<3>{\includegraphics[scale=0.34]{nomajo}}%
...
\only<3->{\includegraphics[height=4.1cm]{u1blc}}
...
\begin{frame}
  ...
  \begin{tabular}{c|c|c|c}
    \hline  
    Fields   & $\operatorname{SU}(2)_L$ & $\operatorname{U}(1)_Y $  & $\operatorname{U}(1)_{\mathcal{D}}^{{\color{Yellow}*}}$\\ \hline
$L $  & $\boldsymbol{2}$& $-1/2$&$0$ \\    
$Q $  & $\boldsymbol{2}$& $-1/6$&$0$ \\
$d_R $& $\boldsymbol{1}$& $-1/2$&$0$ \\
$u_R $& $\boldsymbol{1}$& $+2/3$&$0$ \\
$e_R $& $\boldsymbol{1}$& $-1$  &$0$ \\
$H $  & $\boldsymbol{2}$& $1/2$&  $0$ \\\hline
$\eta$ & $\boldsymbol{2}$& $1/2$  &$1$ \\
$S$ & $\boldsymbol{1}$& $0$ &$2$ \\    
$\sigma$ & $\boldsymbol{1}$& $0$ &$3$ \\\hline        
$\nu_{R1}$& $\boldsymbol{1}$& $0$ & $4$\\
$\nu_{R2}$& $\boldsymbol{1}$& $0$ & $4$\\
$\nu_{R3}$& $\boldsymbol{1}$& $0$ & $-5$\\
$\psi_{R1}$& $\boldsymbol{1}$& $0$ & $-1$\\
$\psi_{R2}$& $\boldsymbol{1}$& $0$ & $-1$\\
$\psi_{R3}$& $\boldsymbol{1}$& $0$ & $-1$\\\hline\hline
TOTAL & & & $0$ \\
  \end{tabular}
      }
    \end{column}
...
\frametitle{Conclusions}
  ...
  \only<1>{
  \begin{block}{Tree-level type-I-II-III seesaws}
    Type-I seesaw: \alert{Multicomponent dark matter}:   In this extension of the SM by an $\operatorname{U}(1)_{B-L}$ gauge symmetry amomalies are canceled partially by two right-handed neutrinos and partially by two component DM Dirac fermions, providing a connection between neutrinos and DM analogous to that one between leptons and quarks in the SM.

    The model predicts the existence of three scalar fields beyond the SM Higgs: $H_1$, $H_2$, $A$

{ \tiny Model implemented in LanHEP. Implemented also in SARAH~{\tiny \url{https://github.com/restrepo/BSM-Submodules/tree/B-L+DM/BSM/SARAH/Models/B-L/DM} (Tested with SARAH-4.14.1)} to analyse perturbativity and stability conditions and higher scales with two-loop RGEs. }
\end{block}
}%
\only<2>{
