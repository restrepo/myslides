...
{mt}%
{}
\arxiv{arXiv:1803.08528}
\collaborators{Mario Reig, Jose Valle (IFIC Valencia), Oscar Zapata (UdeA) }
}

\date{\tiny May 22, 2018 } % (optional) \today
...
\begin{frame}
 ...
 \begin{tabular}{lr}
   ...
   $H$   & 0\\
   $S$   & $s$\\
   $\left( \psi_R \right)^{\dagger}_\alpha$ & $\color{Green}r_{\alpha}$\footnote{Weyl notation with only left-handed fields defined; $\color{Green}r_{\alpha}$ restricted by anomaly cancellation}\\
 \end{tabular}
\end{center}

\begin{block}{Massless Majorana fermions ($n=0,1$)}
  \begin{align*}
    L \left[ \left( \psi_R \right)^{\dagger}_\alpha \left( \psi_R \right)^{\dagger}_\beta S^n \right]  \Longrightarrow& {\color{Green} r_{\alpha}+r_{\beta}+nS\ne 0}\,,
   & &\text{example: $r\ne 1$, if $s=-2$}\,.
  \end{align*}
\end{block}
...
\text{ For $\alpha\le 2$: }   \left( \psi_R \right)^{\dagger}_\alpha\to &\left( \nu_R \right)^{\dagger}_\alpha&
          \color{Green} r_{\alpha}\to & \color{Green} \nu_{\alpha}
 \end{align*}
...
\begin{frame}
 ...
 The required $\operatorname{U}(1)$ symmetry is
 identified with $B-L$
