...
\title{HEP computer tools}
\subtitle{from SARAH and beyond}
...
\begin{frame}
\url{https://www.wolfram.com/download-center/mathematica/}
  
  \url{https://ibug.io/blog/2019/05/mathematica-keygen/}

  \url{https://docs.google.com/presentation/d/1dtaHERLd95FjHqqEhJE2twZGXWPje46yQcic-Y-Qv-U/edit?usp=sharing}

\url{https://clustercien.udea.edu.co/jupyter}
\end{frame}
...
\Large
  \begin{align*}
  L=\frac{1}{2}m {\color{magenta}\boldsymbol{v}\cdot\boldsymbol{v}}-V({\color{magenta}|\boldsymbol{r}|}).
\end{align*}


\end{frame}
\begin{frame}
  \frametitle{Lie groups $\to$ \includegraphics[scale=0.7]{fs}}
...
We want to obtain a $2\times2$ representation of the algebra
\begin{align}
  K=&  \begin{pmatrix}
   0 &-i\\
   -i &0\\ 
...
\end{align}
which can generates an element of the group $\operatorname{SO}(1,1)$ with \emph{\color{red}parameter} ${\color{red}\xi}$ 
\begin{align}
...
The transformation of a timelike and a spacelike coordiantes, are
($c=1$)
...
\begin{frame}%[fragile,allowframebreaks]
\begin{align*}
\cosh{\color{red}\xi}=\gamma= \frac{1}{\sqrt{1-{\color{red}v^2}}}  
\end{align*}
...
\textbf{General}: parameter ${\color{red}\xi}(t,\boldsymbol{x})$ or ${\color{red}v}(t,\boldsymbol{x})$ is local, e.g, accelerated  system invariance: \emph{\color{red}Local} conservation of $E$ and $\boldsymbol{p}$
...
\end{equation}
where $\mu=0,1,2,3$. A {\color{red}pure underscript} 4-vector is
\begin{align}
...
\begin{frame}%[fragile,allowframebreaks]
\frametitle{\small {\color{magenta}Photon}: \only<1>{Representation of the Poincaré Group}\only<2>{$\hat{\boldsymbol{p}}\oplus\operatorname{SO}(1,3)=i\,\partial^{\mu}\oplus$}\only<1>{which transform as a vector under }$\operatorname{SO}(1,3)$\only<2>{ $\to i\,D^{\mu}\oplus\operatorname{SO}(1,3)$   }}
...
\begin{table}
  ...
  \end{tabular}
  \caption{electron {\color{red}left}: $\operatorname{SL}(2,C)\times U(1)_Q$ (subscript)
and positron {\color{red}left}: $\operatorname{SL}(2,C)\times U(1)_Q$ (superscript)
  }
  \label{tab:electron}
\end{table}
...
\end{itemize}
\begin{align*}
\sigma^0=&\mathbb{1}\,,&  \overline{\sigma}^{\mu}=&(\sigma^0,-\boldsymbol{\sigma}), &
...
\begin{table}
  \centering
  \begin{tabular}{llll}
    Name & Symbol & $\operatorname{SL}(2,C)$ & $U(1)_Q$\\\hline
    $e_L$: electron {\color{red}left} & $\xi_{\color{red}\alpha}$ & ${S_{\color{red}\alpha}}^{\color{red}\beta}\xi_{\color{red}\beta}$ & $e^{i\theta}\xi_{\color{red}\alpha}$\\
   $\left( e_L \right)^{\dagger}$: positron {\color{blue}right}   & $\left( \xi_{\color{red}\alpha} \right)^{\dagger}=\xi^{\dagger}_{\color{blue}\dot{\alpha}}$ & $\xi^{\dagger}_{\color{blue}\dot{\beta}}{\left[{S^{\dagger}}\right]^{\color{blue}\dot{\beta}}}_{\color{blue}\dot{\alpha}}$ & $\xi^{\dagger}_{\color{blue}\dot{\alpha}}e^{-i\theta}$\\
   $e_R$: electron {\color{blue}right}   & $\left( \eta^{\color{red}\alpha} \right)^{\dagger}=\eta^{\dagger\;\color{blue}\dot{\alpha}}$ & ${\left[ \left( S^{-1} \right)^\dagger \right]^{\color{blue}\dot{\alpha}}}_{\color{blue}\dot{\beta}}\eta^{\dagger\;\color{blue}\dot{\beta}}$& $e^{i\theta}\eta^{\dagger\;\color{blue}\dot{\alpha}}$\\
   $\left( e_R \right)^{\dagger}$: positron {\color{red}left}&$\eta^{\color{red}\alpha}$& $\eta^{\color{red}\beta}{\left[ S^{-1}  \right]_{\color{red}\beta}}^{\color{red}\alpha}$ & $e^{-i\theta}\eta^{\color{red}\alpha}$\\\hline
...
\begin{align*}
  \xi_{\color{red}\alpha}\to\xi'_{{\color{red}\alpha} }=&e^{i\theta(x)}\xi_{\color{red}\alpha} &   \eta^{\color{red}\alpha}\to\eta^{\,\prime\color{red}\alpha}=&e^{-i\theta(x)}\eta^{\color{red}\alpha} \\
   {\color{magenta}D_\mu} \xi_{\color{red}\alpha}\to\left( {\color{magenta}D_\mu}\xi_{{\color{red}\alpha} }\right)'=&e^{i\theta(x)}{\color{magenta}D_\mu}\xi_{\color{red}\alpha} &  {\color{magenta}D_\mu} \eta^{\color{red}\alpha}\to \left( {\color{magenta}D_\mu}\eta^{\color{red}\alpha} \right)'=&e^{-i\theta(x)}{\color{magenta}D_\mu}\eta^{\color{red}\alpha} \\
\end{align*}
\vspace{-1.5cm}
\begin{align*}
  \mathcal{L}=&i\only<1-2>{\xi^{\dagger}_{\invisible<2->{\color{blue}\dot{\alpha}}}}\only<3->{\left( e_L \right)^{\dagger}}\overline{\sigma}^{\invisible<2->{\mu\;\color{blue}\dot{\alpha}\color{red}\alpha}}{\color{magenta}D_\mu}
\only<1-2>{\xi_{\invisible<2->{\color{red}\alpha}}}\only<3->{e_L}+i\only<1-2>{\eta^{\invisible<2->{\color{red}\alpha}}}\only<3->{\left( e_R \right)^{\dagger}}\sigma^{\mu}_{\invisible<2->{\color{red}\alpha\color{blue}\dot{\alpha}}}{\color{magenta}D_\mu}\only<1-2>{\eta^{\dagger\invisible<2->{\;\color{blue}\dot{\alpha}}}}\only<3->{e_R}
                -m \left[\only<1-2>{\eta^{\invisible<2->{\color{red}\alpha}}}\only<3->{\left( e_R \right)^{\dagger}} \only<1-2>{\xi_{\invisible<2->{\color{red}\alpha}}}\only<3->{e_L}+\only<1-2>{\xi^{\dagger}_{\invisible<2->{\color{blue}\dot{\alpha}}}}\only<3->{\left( e_L \right)^{\dagger}} \only<1-2>{\eta^{\dagger\;\invisible<2->{\color{blue}\dot{\alpha}}}}\only<3->{e_R}\right]-\tfrac{1}{4}{\color{magenta}F^{\mu\nu}}{\color{magenta}F_{\mu\nu}}\\
\invisible<1>{  \mathcal{L}=}&\invisible<1>{i\overline{\psi}\gamma^{\mu}{\color{magenta}D_\mu}\psi -m\overline{\psi}\psi-\tfrac{1}{4}{\color{magenta}F^{\mu\nu}}{\color{magenta}F_{\mu\nu}}\,.}\only<3->{\to e\overline{\psi}\gamma^{\mu}\psi\,{\color{magenta}A_{\mu}}}
\end{align*}
\end{block}
\end{column}
\begin{column}{0.2\textwidth}
  \begin{block}{\invisible<1>{Dirac spinor}}
    \invisible<1>{\only<1-2>{
        \small
    \begin{align*}
      \psi=&
      \begin{pmatrix}
...
\begin{pmatrix}
  ...
  \only<3>{\includegraphics[scale=0.35]{feynmanruleqed}}%
  \only<4>{
    non-bare\\
    \includegraphics[scale=0.35]{5loop}}%
  }
  \end{block}
\end{column}
\end{columns}
...
\begin{frame}
  \frametitle{$\operatorname{SU}(2)_L\invisible<1>{\times{\color{cyan}\operatorname{U}(1)_Y}}\to D_{\mu}=\boldsymbol{1}\partial_{\mu}-ig_2 \frac{\tau_{i}}{2}W_{\mu}^i
    \invisible<1>{{\color{cyan}-ig_1B_{\mu}}}:$ \only<1-5>{17}\only<6>{21}\only<7>{32}\only<8>{62} years later... (stages of grief $\only<1-3>{<}\only<4>{>}\only<5->{\to}$ \only<1-5>{1967}\only<6>{1971}\only<7>{1982}\only<8->{2012})}


  
\begin{columns}
  \begin{column}{0.73\textwidth}
    \begin{block}{\only<1-3>{Not mass, }\only<1>{}\only<2-3>{\color{cyan}hypercharge, }\only<3>{\color{magenta} not Dirac}%
...
\end{columns}
                ...
                {                  \small
                \begin{align*}
                 \hspace{-0.7cm} \mathcal{L}=i \left( L \right)^{\dagger}\overline{\sigma}^{\mu}D_{\mu}L-\tfrac{1}{4}W_{\mu\nu}^iW^{\mu\nu}_{i}
   \invisible<1>{ -{\color{cyan}\tfrac{1}{4}B_{\mu\nu}B^{\mu\nu}}}
 \invisible<1-2>{ -{\color{magenta} i \left( e_R \right)^{\dagger}{\sigma}^{\mu}D_{\mu}e_R }}
 \invisible<1-3>{ + h\left( e_R \right)^{\dagger} \Phi^{\dagger} L
...
\begin{picture}(320,10)  
  \invisible<1-7>{\put(235,8){\includegraphics[scale=0.1]{encircle}}}%
  \invisible<1-7>{\put(355,0){\includegraphics[scale=0.15]{encircle}}}%
\end{picture}
\vspace{-0.5cm}
...
\begin{frame}
  \begin{table}
    \frametitle{First generation of Standard Model (SM)}
    \centering
            \rowcolors{1}{RoyalBlue!20}{}
    \begin{tabular}{|l|c|c|c|l|}\hline
      Field               &Lorentz   & SU(3)$_{C}$            & SU(2)$_{L}$           & U(1)$_Y$          \\\hline
    $Q   $                 & $\xi_{\alpha}^1$ & $\mathbf{3}$           & $\mathbf{2}$          & $1/6$          \\      
    $\color{blue}L   $                 & $\xi_{\alpha}^2$ & $\mathbf{1}$           & $\mathbf{2}$          & $-1/2$          \\
    ...
    $\color{Green}\left(e_R^{-}\right)^{\dagger}$ & $\eta_3^\alpha$  & $\mathbf{1}$            & $\mathbf{1}$            & $1$              \\
     $\color{red}\Phi$                   & -       &  $\color{red}\mathbf{1}$ & $\color{red}\mathbf{2}$ &$\color{red}1/2$\\\hline
  \end{tabular}\hspace{1cm} 
  \caption{Fundamental Fields of SM }
  \label{tab:smflds}
\end{table}
\end{frame}
...
\begin{frame}
\frametitle{Kircchoff's law}
\begin{align}
\color{red}  \widetilde{\Phi}=&\color{red}i\sigma_2 \Phi^{*}=
  \begin{pmatrix}
    {\phi^0}^{*} & -\phi^{-}
  \end{pmatrix}^{\operatorname{T}}
  &\,
   \color{blue} L=&\color{blue}  \begin{pmatrix}
    \nu_L & e_L
  \end{pmatrix}^{\operatorname{T}}\,
\end{align}
\begin{align}
 \invisible<2->{ {\color{Green}\left( e_R \right)^{\dagger}}{\color{red} \Phi^{\dagger}}{\color{blue}L}=    {\color{Green}\left( e_R \right)^{\dagger}}{\color{red} \Phi^{\dagger}}{\color{blue}L}=}
  \only<1>{{\color{Green}\left( e_R \right)^{\dagger}}\epsilon_{\color{red}a\color{blue}b}{\color{red} \widetilde{\Phi}^{a}}{\color{blue}L^{b}}}
  \only<2->{{\color{Green}\left( e_R \right)^{\dagger}}{\color{red} \widetilde{\Phi}}\cdot {\color{blue}L}}\,,
\end{align}
\invisible<1>{Which can be represented as the Kircchoff's law:}\only<2>{}
\begin{columns}
\begin{column}{0.4\textwidth} 
\invisible<1>{\includegraphics[scale=0.5]{eRHL} \hspace{1cm}}
\end{column}
\begin{column}{0.6\textwidth} 
  \begin{align*}
-{\color{Green}Y_R}-{\color{red}Y_\Phi}+{\color{blue}Y_L}=&0\\
\invisible<1>{    {\color{blue}Y_L}=& {\color{red}Y_\Phi}+ {\color{Green}Y_R}}
  \end{align*}
\end{column}
\end{columns}
...
\lstset{
  emph={git,emacs,math,MakeSPheno,MakeCHep,MakeUFO,newProject,SPhenoSSDM,Omega,proton,neutron,SI,CalcOmega,Start},
  emphstyle=\color{red}
  %otherkeywords={git },
}
...
\begin{frame}[fragile]
  ...
  Install compilers:
    ...
    \begin{lstlisting}[language=Sh]
      sudo apt install build-essential gfortran feynmf
    \end{lstlisting}

 Download all the HEP-tools in one step:
...
Gauge[[3]]={G,  SU[3], color,       g3,False,1};
  ...
  \begin{center}
      $B\to B_{\mu},\ \widetilde{B}\qquad\Longrightarrow $\ \texttt{VB}\,,\ \texttt{FB}
  \end{center}
\end{frame}
...
$Q$           &${\color{Green}3}$&$\xi_{1\alpha}: (u_L\ d_L)^{\text{T}}  $&$1/6$ &$\boldsymbol{2}$&$\boldsymbol{3}$&$+$\\ 
$L$           &${\color{Green}3}$&$\xi_{2\alpha}: (\nu_L\ e_L)^{\text{T}}$&$-1/2$&$\boldsymbol{2}$&$\boldsymbol{1}$&$+$\\ 
$d^c$         &${\color{Green}3}$&$\eta_{1}^\alpha: (d_R)^{\dagger}$        &$1/3$ &$\boldsymbol{1}$&$\overline{\boldsymbol{3}}$&$+$\\        
$u^c$         &${\color{Green}3}$&$\eta_{2}^\alpha: (u_R)^{\dagger}$        &$-2/3$&$\boldsymbol{1}$&$\overline{\boldsymbol{3}}$&$+$\\ 
$e^c$         &${\color{Green}3}$&$\eta_{3}^\alpha (e_R)^{\dagger}$        &$1$   &$\boldsymbol{1}$&$\boldsymbol{1}$&$+$\\ \hline
...
FermionFields[[3]] = {dc, 3, conj[dR],     1/3, 1, -3,1};
FermionFields[[4]] = {uc, 3, conj[uR],    -2/3, 1, -3,1};
FermionFields[[5]] = {ec, 3, conj[eR],       1, 1,  1,1};
...
LagHC =   -(Ye ec.conj[H].l + Yd dc.conj[H].q + Yu uc.H.q);
...
DEFINITION[EWSB][MatterSector]=   
    {{{{dL}, {conj[dR]}}, {{DL,Vd}, {DRc,Ud}}},
     {{{uL}, {conj[uR]}}, {{UL,Vu}, {URc,Uu}}},
     {{{eL}, {conj[eR]}}, {{EL,Ve}, {ERc,Ue}}}};  
...
\begin{frame}[fragile]
  \frametitle{Implicit files without editors}
  \begin{lstlisting}[language=Sh, caption=Creates \texttt{kk.txt} file ]
cat << EOF > kk.txt
Hello world
EOF
\end{lstlisting}
...
\begin{lstlisting}[language=Sh, caption=commands expecting input files ]
math << EOF 
2+2
EOF
\end{lstlisting}
...
\begin{frame}[fragile]
  \frametitle{Automatic index contraction}
 \begin{block}{Verifying Index Contractions}
   \begin{columns}
     \begin{column}{0.4\textwidth}
       \begin{align*}
         u^c\cdot H\cdot q=&{\color{magenta}\delta_\alpha^\beta} {\color{blue}\epsilon_{ab}}\, u^{c}_{\color{magenta} \alpha}  H^{\color{blue}a} q^{{\color{magenta}\beta}{\color{blue}b}} \\
         =&{\color{magenta}\overline{\mathbf{3}}\times \mathbf{3}}\otimes{\color{blue}\mathbf{2}\times \mathbf{2}}\,.
       \end{align*}
     \end{column}
     \begin{column}{0.6\textwidth}
         {
    \lstset{basicstyle=\ttfamily\scriptsize,emph={col1,col2,col3,lef1,lef2,lef3,Start,MakeIndexStructure},
      emphstyle=\color{red}}
    \begin{lstlisting}[language=Mathematica]
math<<EOF
<<./SARAH/SARAH.m
Start["SSDM"]
MakeIndexStructure[{u, H, q}] (* uc.H.q *)
EOF      
 ...
Out[3]: Delta[col1, col3] epsTensor[lef2, lef3]
\end{lstlisting}      
    }
       \end{column}
   \end{columns}
 ...
 \begin{block}{Explicit index contraction in \texttt{SSDM.m}}
   {
\lstset{basicstyle=\ttfamily,emph={col1,col2,col3,lef1,lef2,lef3},
      emphstyle=\color{red}}     
    \begin{lstlisting}[language=Mathematica]
   Delta[col1, col3] epsTensor[lef2, lef3]  uc.H.q
 \end{lstlisting}
 }
 \end{block}

 { \tiny See \url{https://gitlab.in2p3.fr/goodsell/sarah/wikis/Automatic_index_contraction}}
...
\section{Automatic generation of scotogenic models }
\begin{frame}[fragile]
  \frametitle{Python program: \texttt{minimal-lagrangians}}
 Simon May, \url{https://arxiv.org/pdf/2003.08037} [CPC]
...
\begin{itemize}
\item As the program was originally written for the study of minimal darkmatter models with radiative neutrino masses: D. Restrepo, O. Zapata, C. E. Yaguna, Models with radiative neutrinomasses and viable dark matter candidates, arXiv:1308.3655 [JHEP]
\item  Lagrangians for the individual models.
\item Model files for SARAH can be constructed automaticallyfrom the specified field content, which can be tedious if done manually
\end{itemize}

Thus, minimal-lagrangians enables rapid phenomenological studies using \texttt{SARAH} and, successively, further tools like \texttt{SPheno}, and \texttt{micrOMEGAs} $\to$ Sec. 8: Outlook
...
\begin{frame}[fragile]
  \begin{lstlisting}[language=Python]
    ...
    ScalarField ("phi",  3, Y= 0),
    FermionField("Ψ'",  2, Y=-1),6), (0, 2)), #α= -2 is equivalent toα= 2
  \end{lstlisting}
...
%%%%
{
\usebackgroundtemplate{\includegraphics[width=\paperwidth]{sara}}
\setbeamertemplate{blocks}[rounded][shadow=false]
\setbeamercovered{invisible}
\begin{frame}[plain]
%\begin{picture}(320,250)
%\only<1>{\put(10,10){  \includegraphics[scale=0.3]{thanks}}}%
%\end{picture}
\end{frame}
%%%%%%%%%%%%%%%%%%
}
...
\begin{picture}(320,250)
  ...
  \only<2>{\put(280,130){\tiny with J. Calle, C. Yaguna, and O. Zapata, arXiv:1812.05523 [PRD]  }}%
    \only<2-4>{\put(-20,130){\tiny
        \begin{minipage}{0.3\linewidth}
          \begin{align*}
            \invisible<4>{\psi_{L,R}\to}&\invisible<4>{\text{Singlet fermions (vector-like)  }}\\
...
\only<4>{\put(40,20){\begin{overpic}[scale=0.6]{radiativedirac3b}\end{overpic}}}%
% 
...
\only<3->{\put(230,185){\tiny
    \begin{minipage}{0.3\linewidth}
      \rowcolors{1}{RoyalBlue!20}{}
  \begin{tabular}{c|c|ccccccc}\hline
 Fields: $f_i$  & $L,e_R$& $\nu_{R3} $
    &$ \nu_{R2}$&$ \nu_{R1}$&\invisible<3>{$\sigma_1^-}$&\invisible<3>{$\sigma_2^-$}&$S$&$H$\\ \hline
  $\operatorname{U}(1)_{B-L}$&$-1$&$\color{Yellow}-4$&$\color{Yellow}-4$&$+5$&\invisible<3>{ $\color{red}-2$} & \invisible<3>{$\color{red}-5$} & $+3$&$0$\\ \hline
  \end{tabular}
         \end{minipage}
       }}%
\only<3->{\put(300,85){\tiny
    \begin{minipage}{0.3\linewidth}
       \begin{block}{\tiny Anomaly cancellation conditions}
         \begin{align*}
           \sum_i \nu_{Ri}=& -3 \\
           \sum_i \nu_{Ri}^3=& -3 \\
         \end{align*}
       \end{block}
       \vspace{-1cm}
         \begin{block}{\tiny Three-level cancellation conditions}
         \begin{align*}
        \cancel{ \nu_R\nu_R} \\
...
\begin{lstlisting}[language=Sh]
cp BSM/Input_Files/LesHouches.in.BLDZ .
\end{lstlisting}
...
\begin{block}{LesHouches.in.BLDZ: Fix options!}
 \texttt{55 1\ \ \ \ \ \ \ \# Calculate loop corrected masses}\\
 \texttt{50 0\ \ \ \ \ \ \ \# Majorana phases: use only positive masses}\\
\texttt{520 0\ \ \ \ \ \ \# Write effective Higgs couplings}
      \end{block}
      \end{minipage}
}}
  \put(100,100){\only<2>{}}
