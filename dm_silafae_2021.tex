...
\title{ Effective Dirac neutrino masses }
\subtitle{with multi-coponent fermionic dark matter}
...
{}
\arxiv{ arXiv:2108.05907
%  arXiv:1812.05523 [PRD], 1906.09685 [PRD], 1907.11938, 1909.09574
}
\collaborators{ Nicolás Bernal
%  Carlos Yaguna (UPTC), Julian Calle, Óscar Zapata, Andrés Rivera (UdeA),\\ Walter Tangarife (Loyola University Chicago)
}
}
...
%         \end{minipage}
%       }}  }}%
\only<3->{\put(250,180){\Large $\color{red}\mathcal{L}=-\frac{1}{4}V_{\mu\nu}V^{\mu\nu}+i \overline{\Psi}\cancel{\mathcal{D}} \Psi- h \overline{\Psi}\Psi S $   }}%
\only<3->{\put(280,218.5){ \Huge {{\color{red}Local}} $\color{red}U(1)_{\mathcal{X}}$  }}%
\only<3->{\put(240,125){
    \begin{minipage}{0.4\linewidth}
     Diracness protected chiral fermion dark matter $\color{red}m_{\Psi}=h\langle S \rangle$
      
          {\color{red}Relic abundance: }
          \begin{align*}
            \text{Active Symmetry: $\color{red}\mathcal{X}\to X$: }&\color{red} \Psi\overline{\Psi}\to \text{SM}\,\text{SM} \\
            \text{Dark Symmetry: $\color{red}\mathcal{X}\to D$: }&\color{red} \Psi\overline{\Psi}\to \gamma_{\mathcal{D}}\gamma_{\mathcal{D}}             
          \end{align*}
    \end{minipage}
  }}%
\only<3->{\put(160,120){\Large $F_{\mu\nu}{\color{red}\ V^{\mu\nu}} $   }}%
\only<3->{\put(120,-10){$\color{red}\overline{\Psi}\Psi=\color{blue}\psi_1\psi_2+\psi_1^\dagger\psi_2^\dagger\to \psi_\alpha\,,\qquad \alpha=1,\ldots N'\to N'>4 $}}
...
\begin{frame}
  \frametitle{Standard model extended with $\operatorname{U}(1)_{\mathcal{X}}$ gauge symmetry}
  \begin{table}
    \centering
        \rowcolors{1}{RoyalBlue!20}{}
  \begin{tabular}{c|c|c|c}
    \hline  
    Fields     & $\operatorname{SU}(2)_L$ & $\operatorname{U}(1)_Y $ & $\operatorname{U}(1)_{\mathcal{X}}$\\ \hline
$L^\dagger $    & $\boldsymbol{2}$& $+1/2$  &  $\color{blue}l$ \\
$Q^\dagger $    & $\boldsymbol{2}$& $-1/6$  &  $q$   \\
$d_R $  & $\boldsymbol{1}$& $-1/2$  &  $d$ \\
$u_R $  & $\boldsymbol{1}$& $+2/3$  &  $u$\\
$e_R $  & $\boldsymbol{1}$& $-1$    &  $e$   \\
$H $    & $\boldsymbol{2}$& $-1/2$  &  $h$   \\
$\psi_\alpha $ & $\boldsymbol{1}$& $0$     &  $n_\alpha$   \\
  \end{tabular}
  \caption{$l=q=d=u=e=h=0$ for $\mathcal{X}=D$ }
    \label{tab:partcont2}
\end{table}
...
[2 {\color{blue}l}+3 \cdot 2 q]=&0 \nonumber\\
\left[\mathrm{U}(1)_{Y}\right]^{2} \mathrm{U}(1)_{X} :&&
{ \left[(-2)^{2} e+3\left(\tfrac{4}{3}\right)^{2} u+3\left(-\tfrac{2}{3}\right)^{2} d\right]-\left[2(+1)^{2} {\color{blue}l}+3 \cdot 2\left(-\tfrac{1}{3}\right)^{2} q\right]}=&
 0
  \end{align*}}
\only<2->{
with solution 
\begin{align*}
  u=&\only<2-4>{-e-\frac{2{\color{blue}l}}{3}}\only<5->{-\only<5->{{\color{Green}m}}+\frac{4{\color{blue}l}}{3}}\,,& d=& \only<2-4>{e+\frac{4{\color{blue}l}}{3}}\only<5->{\only<5->{{\color{Green}m}}-\frac{2{\color{blue}l}}{3}}\,,& q=& -\frac{{\color{blue}l}}{3}\,,
  &\invisible<2-4>{e=}&\invisible<2-4>{\only<5->{{\color{Green}m}}-2{\color{blue}l}\,,}
\end{align*}}
\invisible<1-2>{
which satisfy
\begin{align*}
\mathrm{U}(1)_{Y}\left[\mathrm{U}(1)_{X}\right]^{2} :&&  
\left[(-2) e^{2}+3\left(\tfrac{4}{3}\right) u^{2}+3\left(-\tfrac{2}{3}\right) d^{2}\right]-\left[2(+1) {\color{blue}l}^2+3 \cdot 2\left(-\tfrac{1}{3}\right) q^{2}\right]=&0
\end{align*}
}
\invisible<1-3>{
  For $N$ extra quiral fields $\psi_\alpha$ ($\alpha=1,\ldots,N'$) with $\mathcal{X}$-charges $n_\alpha$:
  \invisible<4>{${\color{Green}m}\equiv e-2{\color{blue}l}$}
  \begin{align}
    \label{eq:x}
  \left[\mathrm{SO}(1,3)\right]^{2} \mathrm{U}(1)_{\mathcal{X}}:&&  \sum_{\alpha} n_{\alpha}+3 \only<4>{(e-2{\color{blue}l})}\only<5->{{\color{Green}m}}=&0\,,\\
    \label{eq:x3}
    \left[\mathrm{U}(1)_{\mathcal{X}}\right]^{3}\,,&&\sum_{\alpha} n_{\alpha}^3+3 \only<4>{(e-2{\color{blue}l})^3}\only<5->{{\color{Green}m^3}}=&0\,    
\end{align}
}
%
\invisible<1-5>{\only<6->{
    Any set of integers of integers $(n_1,n_2,\ldots n_N)$ which satisfy \eqref{eq:x} and \eqref{eq:x3} can be interpreted as $\mathcal{X}\to D$ symmetry with $N$  chiral fields.

    If one integer, $\color{Green}m$ is repetead 3 times, the set can be interpreted as $\mathcal{X}\to X$ symmetry with $N'=N-3$  chiral fields.
}
}
\end{frame}
...
{
\usebackgroundtemplate{\includegraphics[width=0.8\paperwidth]{ano1}}
...
{
\usebackgroundtemplate{\includegraphics[width=0.8\paperwidth]{ano2}}
\setbeamertemplate{blocks}[rounded][shadow=false]
\setbeamercovered{invisible}
\begin{frame}[plain]
\end{frame}
...
{
\usebackgroundtemplate{\includegraphics[width=0.8\paperwidth]{ano3}}
\setbeamertemplate{blocks}[rounded][shadow=false]
\setbeamercovered{invisible}
\begin{frame}[plain]
\end{frame}
...
\begin{align*} 
    \mathcal{L}_{\text{eff}} = h_{\nu}^{\alpha i} \, \left( \nu_{R\alpha}\right)^{\dagger} \, \epsilon_{ab} \, L_i^a \, H^b \left(\frac{S^*}{\Lambda}\right)^\delta + \text{H.c.},\qquad \text{with $i=1,2,3$}\,,
\end{align*}
and $\delta = 1,2,\ldots$ for dimension 5 (D-5) or 6 (D-6)   operators, etc.
Here $h_{\nu}^{\alpha i}$ correspond to dimensionless induced couplings, $\nu_{R\alpha}$ are at least two RHNs ($\alpha=1,2,\ldots$) with the same $D$ or $X$-charge $\nu$, $L_{i}$ are the lepton doublets with $X$-charge $-L$, $H$ is the SM Higgs doublet with $X$-charge $h=L-m$, $S$ is the complex singlet scalar responsible for the SSB of the anomaly-free gauge symmetry with $D$ or $X$-charge 
\begin{align*}
    s=-(\nu+m)/\delta\,,
\end{align*}
...
\begin{frame}
  \frametitle{Diracness of non-zero DM and Dirac neutrinos masses from $U(1)_{\mathcal{X}}$}

  Starting from the extended dataset with the solutions with $N$ integers to the Diophantine equations~\eqref{eq:x} \eqref{eq:x3}, we apply the following steps
\begin{itemize}
...
\begin{frame}
  \frametitle{Unconditional stability}

Two DM candidates with \emph{unconditional} stability. This happens when there are two remnant symmetries such that $\mathbb{Z}_{|s|}\cong \mathbb{Z}_p\otimes\mathbb{Z}_q$ with  $p$ and $q$ coprimes and $|s|=pq$, which guarantee the stability of each lightest state under $\mathbb{Z}_p$ and $\mathbb{Z}_q$ respectively, without imposing any kinematical restriction. For the two DM candidates associated to the set of chiral fields $\psi_i$ and $\chi_j$, we consider below the following two possibilities for $|s|$
\begin{itemize}
    \item $\mathbb{Z}_{6}\cong \mathbb{Z}_2\otimes\mathbb{Z}_3$: solutions with at least a set of chiral fields with $\psi_i \sim\left[\omega_{6}^{2} \vee \omega_{6}^{4}\right]$ under $\mathbb{Z}_{6}$, and at least a set of chiral fields with $\chi_i \sim \omega_{6}^3$ under $\mathbb{Z}_{6}$,
%
    \item $\mathbb{Z}_{14}\cong \mathbb{Z}_2\otimes\mathbb{Z}_7$: solutions with at least a set of chiral fields with $\psi_i \sim\left[\omega_{14}^{2}  \vee \omega_{14}^{6} \vee \omega_{14}^{8} \vee \omega_{14}^{10} \vee \omega_{14}^{12}\right]$ under $\mathbb{Z}_{14}$ and at least a set of chiral fields with $\chi_i \sim \omega_{14}^7$ under $\mathbb{Z}_{14}$,
\end{itemize}
where $\omega_{|s|}=\operatorname{e}^{i\, 2\pi/|s|}$.  
\end{frame}
...
\begin{table}
  ...
  \begin{tabular}{l|c|c|c|c                                  }\hline
    ...
    $e_{Ri}$         & $\mathbf{1}$ & $-2$   & $3-2L$ & $-1$     \\ %-3/3
    $\nu_{R \alpha}$ & $\mathbf{1}$ & $0$      & $-5$ & $-5/3$     \\ %-5/3
    $\psi_1$         & $\mathbf{1}$ & $0$      & $-7 $ & $-7/3 $  \\ 
    $\psi_2$         & $\mathbf{1}$ & $0$      & $8$ & $8/3$    \\
    $H$              & $\mathbf{2}$ & $1/2$    &  $L-3 $ &$0$  \\        
    $S$              & $\mathbf{1}$ & $0$      &  $1$ & $1/3$   \\\hline 
    $\sigma_1^+$     & $\mathbf{1}$ & $+2$    &  $2L $ & $2$  \\ %6/3       
    $\sigma_2^+$     & $\mathbf{1}$ & $+2$    &  $-(2-2L) $ & $4/3$  \\\hline   %2L+2-2L=2     
  \end{tabular}
  \caption{Charges for last solution. $i=1,2,3$, $\alpha=1,2,3$. Note that $\left( \omega_n^d \right)^{*}=\omega_n^{-d}=\omega_n^{n-d}$.}
  \label{tab:pickedsltn}
\end{table}

Neutrino phenomenology with J. Calle and O. Zapata: arXiv:2103.15328  [PRD]

DM Phenomenology: arXiv:1506.05107
\end{frame}
