...
\title{Secluded-Scotogenic (SS) models}
\subtitle{}
...
\institute{
  \begin{picture}(100,2)
    \put(230,-99){ \includegraphics[scale=0.11]{comhep2024}
    }
\end{picture}
...
\begin{frame}
  \frametitle{DM Zoo}
\centering
\includegraphics[scale=0.29]{dmzoo}
\end{frame}
\begin{frame}
  \frametitle{DM Zoo}
\centering
\includegraphics[scale=0.29]{dmzoo2}
\end{frame}

%%%%%%%%%% 

\begin{frame}
  \frametitle{Simple WIMP dark matter model: Inert doublet model: \hfill \small Deshpande\&Ma'77}
...
{
  \usebackgroundtemplate{\includegraphics[width=\paperwidth]{  effectivenuMD  }}%\only<2>{effectivenuMD}}}
  \begin{frame}
...
\begin{frame}
  \frametitle{$SU(3)\times SU(2)$ \hfill  arXiv:2409.09008}
  \small
  \begin{align*}
    Q'(3,2) =&
               \begin{pmatrix}
                 \color{red}\sqrt{3} a \lambda^8_{11} + b \lambda^3_{11} + 2 c \sigma^3_{11}\\
                 \color{red} \sqrt{3} a \lambda^8_{11} + b \lambda^3_{11} - 2 c \sigma^3_{11}\\                 
               \end{pmatrix},\quad
    \begin{pmatrix}
      \color{blue}\sqrt{3} a \lambda^8_{22} + b \lambda^3_{22} + 2 c \sigma^3_{22}\\
      \color{blue}\sqrt{3} a \lambda^8_{22} + b \lambda^3_{22} - 2 c \sigma^3_{22},\\
    \end{pmatrix},\quad
    \begin{pmatrix}
      \color{Green}\sqrt{3} a \lambda^8_{33} + b \lambda^3_{33} + 2 c \sigma^3_{33}\\
     \color{Green} \sqrt{3} a \lambda^8_{33} + b \lambda^3_{33} - 2 c \sigma^3_{33},\\ 
    \end{pmatrix}\\
 \bar{u}(3^*,1)=  & \color{red}-\sqrt{3} a \lambda^8_{11} - b \lambda^3_{11},\quad
    \color{blue}-\sqrt{3} a \lambda^8_{22} - b \lambda^3_{22},\quad
    \color{Green}-\sqrt{3} a \lambda^8_{33} - b \lambda^3_{33}\\
 \bar{d}(3^*,1)=   & \color{red}-\sqrt{3} a \lambda^8_{11} - b \lambda^3_{11},\quad
    \color{blue}-\sqrt{3} a \lambda^8_{22} - b \lambda^3_{22},\quad
    \color{Green}-\sqrt{3} a \lambda^8_{33} - b \lambda^3_{33}
  \end{align*}

  \begin{align*}
      S = [\color{red}a + b + c, \color{blue}a + b - c, a - b + c, \color{Green}a - b - c, -2a + c, -2a - c,
\color{red}-a - b,\color{blue} -a - b, \color{Green}-a + b,\color{red} -a + b, \color{blue}-2a,\color{Green} 2a]
  \end{align*}
...
\begin{frame}
   ...
   \begin{itemize}%[4,4,-5,-1,-1,-1] like with a massles protected fermion which does not thermalize
     \item \emph{\color{red}Dark Higgs mechanism}: Singlet dark Higgs $\color{Green}\phi$ acquires a vev and give mass to the dark photon
       \begin{align}
  \mathcal{L}=i \psi_{a}^{\dagger}\overline{\sigma^\mu}\left(\partial_\mu -i g_D Z^{\prime}_\mu \right)\psi_{a}-\frac{1}{4}V_{\mu\nu} V^{ \mu\nu }+\color{blue} \sum_{a<b}h_{ab} \psi_{a}\psi_b {\color{Green}\phi^{(*)}} +\text{h.c} {\color{Green}-V(\phi)}\,.
       \end{align}
   ...
   \item  $S_\alpha$  are the charges of SM-singlet right-handed chiral fermions with $N\ge 5$
     ...
     \item $\psi_a$ \emph{\color{red}multi-component dark matter}: massive after
       ...
       with $a=N'+1,\cdots, N$
     \end{itemize}
   \item \emph{\color{red}Larger parameter space}:  Dark photon, $Z^{\prime}$, exclusions instead of $B-L$-like $Z'$ 
   \item Two mediators with $Z'$ and $\color{Green}\phi$ masses are related by (arXiv:1610.03063)
     \begin{align*}
       h/g_D = \sqrt{2}m_\psi /m_{Z'}\,.
     \end{align*}
   \end{itemize}
\end{frame}
...
of the kinetic terms
      \begin{align*}
        \mathcal{L}_{\text{mix}} \supset Z_{\mu}^{\prime} (\epsilon eJ_{EM}^{\mu} + g_{D}J_{D}^{\mu})
      \end{align*}
...
\vspace{-0.5cm}
  \begin{align*}
    \left[\psi_{1},\psi_2,\cdots,\psi_{N}\right], \qquad N_{\text{min}} = 5
  \end{align*}
...
\vspace{0.2cm}
$\sim 1\,000$ solutions $[x,y,\cdots]$ {\small with $N\ge 8$}
...
\vspace{-1cm}
  \begin{align*}
    \left[{\color{red}\nu,\nu, (\nu)}, \psi_{1},\psi_2,\cdots,\psi_{N'} \right], \qquad   N = {\color{red}N_\nu} + N'
  \end{align*}
...
\vspace{0.2cm}
$\sim 1\;000$ solutions $[{\color{red}\nu,\nu,(\nu)},x,y,\cdots]$ {\small with $N\ge 6$}
